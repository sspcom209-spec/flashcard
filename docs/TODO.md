...
**Phase 1 verified:** `npm run dev` starts Vite at http://localhost:5173; `src/components`, `src/data`, `src/pages` exist; `src/data/flashcards.ts` exports 4 cards per category (animals, food, verbs) with `spanish`, `english`, `category`, `quiz`; Home links go to `/study/category`, `/quiz/category`, `/stats`.
